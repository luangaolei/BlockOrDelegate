...
/*
 ...
 - (void)aaabbb:(myBlock)mb;
 
 2.然后再代理方页面,做类似给block创建代理关系的操作,一般都在按钮点击事件,push到委托方（下一页）的操作那里
 [委托方对象 aaabbb:^(NSString *str){
	NSLog(@“%@“, str);//第二步的时候不走这里面的操作(NSLog不执行)
 }];
 
 3.到委托方页面里，在pop回前一页的按钮点击事件里给block赋值,进行传值
 实现.h中声明的- (void)aaabbb:(myBlock)mb;方法
...
- (void)push{
    ...
    [two changeViewColor:^(UIColor *color) {
